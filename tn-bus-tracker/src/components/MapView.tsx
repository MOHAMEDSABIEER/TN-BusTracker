...
export const MapView: React.FC<MapViewProps> = ({ route, isSimulating }) => {
  ...
  return (
    <div className="h-full w-full relative">
      <MapContainer center={[route.stops[0].lat, route.stops[0].lng]} zoom={13} scrollWheelZoom={false}>
        ...
        <Polyline positions={polylinePositions} color="#0f766e" weight={4} opacity={0.6} dashArray="10, 10" />
          ...
          return (
            <Marker key={idx} position={[stop.lat, stop.lng]} icon={icon}>
              <Popup>
                ...
                <div className="text-xs text-slate-500">Scheduled: {stop.time}</div>
              </Popup>
            </Marker>
          );
        })}
        ...
        {isSimulating && (
          <Marker position={busPos} icon={BusIcon}>
            <Popup>
              <div className="font-bold text-teal-700">Bus is here</div>
              <div className="text-xs">Heading to: {route.stops[nextStopIndex].name}</div>
            </Popup>
          </Marker>
        )}
      </MapContainer>
      ...
      {/* Map Legend */}
      <div className="absolute top-4 right-4 bg-white/95 backdrop-blur p-3 rounded-xl shadow-lg z-[1000] border border-slate-100 text-[10px] space-y-2 min-w-[120px]">
        <p className="font-bold text-slate-400 uppercase tracking-widest mb-1">Legend</p>
        <div className="flex items-center gap-2">
          <div className="w-4 h-0.5 bg-teal-700 border-t-2 border-dashed border-teal-700"></div>
          <span className="font-medium text-slate-600">Route Path</span>
        </div>
        <div className="flex items-center gap-2">
          <div className="w-3 h-3 rounded-full bg-teal-700 border border-white"></div>
          <span className="font-medium text-slate-600">Origin Stop</span>
        </div>
        <div className="flex items-center gap-2">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="font-medium text-slate-600">Intermediate</span>
        </div>
        {isSimulating && (
          <div className="flex items-center gap-2">
            ...
            <span className="font-medium text-slate-600">Current Bus</span>
          </div>
        )}
      </div>

      {isSimulating && (
        <div className="absolute bottom-4 left-4 right-4 bg-white/90 backdrop-blur p-3 rounded-xl shadow-lg z-[1000] border border-teal-100">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className="bg-teal-100 p-2 rounded-lg">
                <BusFront className="w-5 h-5 text-teal-700 animate-pulse" />
              </div>
              <div>
                <p className="text-xs font-medium text-slate-500 uppercase tracking-wider">Next Stop</p>
                <p className="font-bold text-slate-800">{route.stops[nextStopIndex].name}</p>
              </div>
            </div>
            <div className="text-right">
              <p className="text-xs font-medium text-slate-500 uppercase tracking-wider">ETA</p>
              <p className="font-bold text-teal-600">{Math.ceil((1 - progress) * 5)} mins</p>
            </div>
          </div>
        </div>
      )}
